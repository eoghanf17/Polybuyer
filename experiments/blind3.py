"""Blind test 3: do the tradeable markets have findable posts?
...
def main() -> None:
    ...
    trades = json.load(open("experiments/refined.json"))["trades"]
    res = sorted([t for t in trades if t["kind"] == "resolving"],
                 key=lambda t: -t["pnl_usd"])
    seen, shortlist = set(), []
    for t in res:
        if t["cid"] in seen:
            continue
        rule = next((r for frag, r in RULES.items() if frag.lower() in t["q"].lower()), None)
        if rule is None:
            continue
        seen.add(t["cid"])
        shortlist.append({**t, "rule": rule})
    ...
    for m in shortlist:
        ...
        for p in posts:
            corpus_rows.append(C.Post(
                ...
                source="blind3/depth_first", query=m["rule"]))
        results.append({**{k: v for k, v in m.items() if k != "moves"},
                        "by_floor": by_floor})
        time.sleep(1)

    C.add(corpus_rows)
    json.dump(results, open("experiments/blind3_result.json", "w"), indent=1)
    print(f"\n{'='*70}")
    for floor in FLOORS:
